...
function ProductComponent({
  ...
  const getShipmentData = async (offer) => {
    if (offer !== undefined) {
      const postData = {
        product: ssrData.product?.id,
        seller_id: offer?.marketplace_seller_id?.toString(),
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && history.isReady) {
      ...
      if (offerDataGql !== undefined && offerDataGql !== null) {
        if (appLocationOffer === "true") {
          const locationOffer = [...offerDataGql.offers].sort(
            (a, b) => a.distance - b.distance
          );

          setOfferData(locationOffer[0]);

          getPortions(locationOffer[0]);
          if (located && locationOffer.length > 0) {
            getShipmentData(locationOffer[0]);
          } else {
            setShipment(false);
            setShipmentLoading(false);
          }
          if (
            ...
            offerDataGql !== undefined &&
            offerData !== null
          ) {
            getListWishList();
            getTerm();
            getIdOffer(locationOffer);
          }
        } else {
          setOfferData(offerDataGql.offers[0]);
          getShipmentData(offerDataGql.offers[0]);
          getPortions(offerDataGql.offers[0]);
          ...
          if (
            ...
            getListWishList();
            getTerm();
            getIdOffer(offerDataGql.offers);
          }
        }
      } else {
        setTimeout(function () {
  ...
  }, [slug, varianteData, offerDataGql]);

  async function getPortions(offer) {
    try {
      const response = await api.post("/getinstallment", {
        value: offer.of_to_view ? offer.promotional_price : offer.price,
      });
  ...
  function validarVariante([valorEnviado]) {
    ...
    if (reduceDoClique.length > 1) {
      ...
      if (posicaoArray.length > 1) {
        ...
        history.push(`/product/${reduceDoClique[posicaoArray[0]].url}`);
        setShipmentLoading(true);
      }
    } else {
      history.push(`/product/${reduceDoClique[0].url}`);
      setShipmentLoading(true);
    }
  }
          ...
          [e.currentTarget.getAttribute("loja")]: [
            {
              ...
              seller_info: {
                ...
                offer: parseInt(e.currentTarget.getAttribute("oferta")),
                store: e.currentTarget.getAttribute("loja"),
                store_url: offerData.marketplace_seller[0].url,
              },
            },
          ],
        },
              ...
              <div className="caixaLista">
                ...
                >
                  ...
                  <div className="containerDados">
                    <div className="titleCard">Adicionar uma lista nova</div>
                          ...
                          <div className="dataProduct">
                            ...
                            <S.titulo>
                              <S.Flex>
                                {offerLoading ? (
                                  ...
                                  <S.ContainerPrice>
                                    <div className="beforePrice">
                                      {offerData.of_to_view &&
                                        `
                                        ${currencyFormat(offerData.price)}`}
                                    </div>
                                    <div className="containerAfterPrice">
                                      <h2 className="price">
                                        {offerData.of_to_view
                                          ? currencyFormat(
                                              offerData.promotional_price
                                            )
                                          : currencyFormat(offerData.price)}
                                      </h2>
                                      {offerData.of_to_view && (
                                        <div className="percentagePrice">
                                          - {offerData.promotional_percentage} %
                                        </div>
                                      )}
                                    </div>
                                    {portions !== false &&
                                ...
                                {offerData && appLocationOffer === "true" ? (
                                  <AddCart
                                    qtd={quantidade}
                                    setQtd={setQuantidade}
                                    oferta={offerData.id}
                                    loja={
                                      offerData.marketplace_seller !== undefined
                                        ? offerData?.marketplace_seller[0]
                                            .shop_title
                                        : ""
                                    }
                                    seller={offerData.marketplace_seller_id}
                                    valor={
                                      offerData.of_to_view
                                        ? offerData.promotional_price
                                        : offerData.price
                                          ...
                                          {offerData && (
                                            <>
                                              ...
                                              <a
                                                ...
                                                href={`/seller/${offerData.marketplace_seller[0].url}`}
                                              >
                                                <h4>
                                                  {
                                                    offerData
                ...
                <Container>
                  ...
                  {relatedData !== undefined &&
                    relatedData.related.length > 0 && (
                      <S.productsContainer>
                        <h3 className="subtitle">Outros modelos semelhantes</h3>
